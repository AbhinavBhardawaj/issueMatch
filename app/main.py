...
def get_application_scout_service() -> ScoutService:
    ...
    if _scout_service is None:
        ...
        # Authenticator for installation clients
        ...
        async def read_client_factory(installation_id: int):
        ...
        verifier_pipeline = VerifierPipeline(
            ...
            github_write_client=None,  # Configured per run or uses client
        ...
        _scout_service = ScoutService(
            ...
            downstream_write_client_factory=read_client_factory,
        )
...
def create_app(
    ...
    store = delivery_store if delivery_store is not None else InMemoryDeliveryStore()
...
# Default instance for ASGI servers
app = FastAPI(title="IssueMatch")
if os.getenv("ENABLE_DEV_VERIFY_API", "").lower() in ("true", "1", "yes"):
    app.include_router(verify_router, prefix="/api")
if os.getenv("ENABLE_DEV_SCOUT_API", "").lower() in ("true", "1", "yes"):
