...
# PV9
def test_pv_repo_context_wrong_installation(make_finding, make_verification, make_evidence_result, make_repo_context):
    f = make_finding()
    v = make_verification(finding=f)
    er = make_evidence_result(finding=f)
    rc = make_repo_context(installation_id="I-999")

    res = post_verifier_recheck(f, v, er, rc)
    assert res.status == RecheckStatus.FAIL
    assert res.reason == "REPO_CONTEXT_INSTALLATION_MISMATCH"
