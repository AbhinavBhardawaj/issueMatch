...
class FakeGitHubClient:
    def __init__(self, fail: bool = False, files: dict[str, str] | None = None): self.fail, self.files = fail, files or {"src/auth/session.py": "def timeout(): pass", "tests/test_session.py": "def test_timeout(): pass"}
    def _check(self):
...
class FakeAnalysisService:
    ...
    async def analyze(self, candidate, context):
        ...
        if self.fail: raise RuntimeError("analysis unavailable")
